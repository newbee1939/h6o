...
<!-- TODO: 最後に全部再度見返してレビューしてから投稿（英語版も） -->
<!-- できればHardも解きたい -->
...
最後に入れたものから先に取り出す入れ物（**LIFO** = Last In, First Out）。JavaScript では配列の `push` / `pop` がそのままスタックになる。

効くのは**入れ子（ネスト）を扱うとき**。「いちばん内側から順に片付く」構造は、そのまま「最後に積んだものから順に降ろす」に一致する。括弧の対応、HTML タグの開閉、関数呼び出し（プログラムが動くとき OS が用意する**コールスタック**そのもの）——全部これ。
...
`"([{"` まで読んだ時点で、次に来てよい閉じ括弧は `}` ただ1つ。`}` を閉じたら次は `]`、その次が `)`。つまり**開いた順とは逆順に閉じる**。

この「逆順に思い出す」がスタックの定義そのもの。変数1本では持てないのがここで、覚えるべきは**まだ閉じていない開き括弧の列**であり、その長さは入力しだいでいくらでも伸びる。
...
c='['  表に無い -> 積む          stack = ['[']
c='('  表に無い -> 積む          stack = ['[', '(']
c=']'  open='['  pop() は '('    -> 不一致。ここで false
                                    ('(' がまだ閉じていないのに ']' が来た)
```

`"([{}])"` なら、`{` `[` `(` の順に払い出されて最後に `stack` が空になり true。

**応用**: 「**入れ子になっていて、いちばん内側から片付く**」と気づいたらスタック。逆に言えば、**直前の1つだけ覚えれば足りる問題はスタックではない**（変数で済む）。覚える量が入力しだいで伸びるかどうかが分かれ目。
