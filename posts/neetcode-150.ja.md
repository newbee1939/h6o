...
- **`l` は1つずつ動かない**。より安い日が来たときだけ**そこへ飛ぶ**（窓を縮めるのではなく、そこから作り直す）
...
- **`maxProfit` を `-Infinity` で始める**: 全日下落のとき負の値が返る。「取引しない」が許されているので初期値は 0
- **`minBuy` を先に更新する**: 同じ日に買って売る形になる。この問題は利益 0 なので答えは変わらないが、順序に意味があることは意識しておく
