...
O(n²)。ソートの O(n log n) は飲み込まれる。

#### 動きを1本追う

`[-1,0,1,2,-1,-4]` を sort した `[-4, -1, -1, 0, 1, 2]`（添字 0〜5）。
...
`i` は 3 で打ち止め（`i < 6 - 2`）。**`i = 4` だと `l = r = 5` で `while` が1回も回らない**。これが `- 2` の正体。
...
```
1行が無いと: [[-1,-1,2], [-1,0,1], [-1,0,1]]   ← 重複
1行があると: [[-1,-1,2], [-1,0,1]]
```

表の `i = 1` と `i = 2` を見比べると理由が出る。**的が同じ 1 で、範囲は `i = 1` の一部**。同じ的を狭い場所で探し直しているので、新しい組は原理的に出ない。

**「範囲が狭いから冗長」ではない**点に注意。値が変われば的も変わるので、狭い範囲から新しい組が出る:

| i | nums[i] | 的 | 探す範囲 | 出る組 |
|---|---|---|---|---|
| 0 | -3 | 3 | `[-1, 0, 1, 4]` | `[-3,-1,4]` |
| 1 | -1 | **1** | `[0, 1, 4]` ← 狭い | `[-1,0,1]` ← **新しい組** |

（入力 `[-3, -1, 0, 1, 4]`）

つまり潰しているのは「**同じ的を2回狙う回**」だけ。同じ値が3つ以上でも同じで、`i` は「1番目に使う」役だけを持ち、**同じ値を2個使う組は `l`、3個使う組は `l` と `r`** が引き受ける。`[0,0,0]` は `i=0, l=1, r=2` で出る（`0` で `break` すると落ちるので、判定は `>= 0` ではなく `> 0`）。
